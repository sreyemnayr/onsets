...
export class PermutationsService {
  ...
  hash_to_string(hash) {
    switch (hash) {
      ...
      case FACES.UNION:
        return '∪';
      case FACES.INTERSECT:
        return '∩';
      case FACES.DIFFERENCE:
        return '—';
      case FACES.COMPLEMENT:
        return '´';
      case FACES.UNIVERSE:
        return 'V';
      case FACES.EMPTY:
  ...
  checkValidCombo(combo, required = []) {
    ...
    combo.forEach(c => {
      ...
      count[PermutationsService.checkType(c)]++;
    });
    if (r.length > 0) {
      return false;
    }
    ...
    if (count[OPERATOR] >= count[IDENTIFIER]) {
      return false;
    }
    return count[IDENTIFIER] <= count[OPERATOR] + 1;
  }
  ...
  checkValidPermutation(permutation: Array<CubeComponent>) {
    if (PermutationsService.checkType(permutation[0]) !== IDENTIFIER) {
      return false;
    } else {
    ...
    if (permutation.length > 1) {
      for (let i = 1; i < permutation.length; i++) {
        if (PermutationsService.checkType(permutation[i]) === MODIFIER) {
          if (
            PermutationsService.checkType(permutation[i - 1]) !== IDENTIFIER &&
            PermutationsService.checkType(permutation[i - 1]) !== MODIFIER
          ) {
            return false;
          }
        }
        if (PermutationsService.checkType(permutation[i]) === IDENTIFIER) {
          if (PermutationsService.checkType(permutation[i - 1]) !== OPERATOR) {
            return false;
          }
        }
        if (PermutationsService.checkType(permutation[i]) === OPERATOR) {
          if (
            PermutationsService.checkType(permutation[i - 1]) !== IDENTIFIER &&
            PermutationsService.checkType(permutation[i - 1]) !== MODIFIER
          ) {
  ...
  *permute(permutation) {
    ...
    if (this.checkValidPermutation(to_yield)) {
      yield to_yield;
    }
    while (i < length) {
      if (c[i] < i) {
